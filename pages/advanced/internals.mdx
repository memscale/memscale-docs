...
[API Reference](/reference/overview) — `wrap`, `optimize`, `detach`, `Config`,
...
3. **`core.executor` — `Executor`.** Applies the plan by attaching hooks,
   ...
   [`detach()`](/reference/detach) can reverse everything.
...
what makes [`detach()`](/reference/detach) and the [`optimize()`](/reference/optimize)
