...
| -------- | ------- | ------- |
| [`wrap(target, config=None, ...)`](/api/wrap) | Optimize a model or `Trainer` | the same object, with hooks attached |
| [`optimize(model, optimizer=None, ...)`](/api/optimize) | Context manager for custom training loops | yields the executor |
| [`detach(model)`](/api/detach) | Remove MemScale from a model | `None` |

## Configuration

| Object | Purpose |
| ------ | ------- |
| [`Config`](/api/config) | All optimization settings |
